...
- Not verified end to end: the auto-relaunch path deliberately does nothing in a dev build (a dev build is replaced by every `run.sh`, which restarts it itself), so proving it needs a signed release replacing `/Applications` while running. Do that as the release check for 0.1.17.
